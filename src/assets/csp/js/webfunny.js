        ...
        function r(e, t) {
          if (f[e] && !0 !== f[e].uploadFlag) {
            ...
            if (i && -1 == i.indexOf(p) && -1 == i.indexOf(h)) {
              var d = new V(v, o, i, s, a, '发起请求', '', f[e].timeStamp, 0);
              d.handleLogInfo(v, d);
              var u = new V(v, o, i, s, a, '请求返回', n, r, c);
        ...
        (l.webfunny = {
          wm_check: function() {
            ...
            return (
              ...
              console.log('身份标识：' + d.wmUserInfo),
              console.log('探针标识：' + c),
              console.log('上报接口：' + y),
              console.log('启动列表：' + d.sl),
              console.log('...........'),
              console.log('PVUV状态：' + (_ || e)),
              console.log('静态资源状态：' + (P || e)),
              console.log('JS错误状态：' + (U || e)),
              console.log('接口请求状态：' + (I || e)),
              console.log('页面加载状态：' + (C || e)),
              console.log('行为状态：' + (N || e)),
              console.log('信息初始化状态：' + (A || '未初始化！部分功能将无法使用，请查看文档(API方法调用)')),
              'end'
            );
          },
          ...
          wm_init_user: function(e, t, n) {
            return (
              e || console.warn('userId 初始化值为0(不推荐) 或者 未初始化'),
              n || console.warn('secondParam 初始化值为0(不推荐) 或者 未初始化'),
              t && (c = t + '_webmonitor'),
          ...
          wmInitUser: function(e, t) {
            return (
              e || console.warn('userId(用户唯一标识) 初始化值为0(不推荐) 或者 未传值, 探针可能无法生效'),
              t || console.warn('projectVersion(应用版本号) 初始化值为0(不推荐) 或者 未传值, 探针可能无法生效'),
              (d.wmUserInfo = JSON.stringify({
                userId: e,
                projectVersion: t,
              })),
              (A = '用户信息初始化：userId=' + e + '，版本号：' + t),
              1
            );
          },

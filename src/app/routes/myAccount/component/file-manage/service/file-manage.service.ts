...
export class FileManageService {
  ...
  ): Observable<SharingFile[]> {
    return this.http.get('/CSP/Attachment/GetList',
      { BusinessId, BusinessType, AttachmentType })
      .pipe(map((o: any) => o.items)) as any;
  }
...
/**
  ...
  upload(file: File) {
    ...
    return this.http.post(`/Storage/File/Upload`, formData, null, {headers:new HttpHeaders()});
  }
  ...
  getAvatarUrl(option: { idOrUrl?: string, defaultUrl?: string, fromAbp?: boolean }) {
    ...
    if (fromAbp) {
      idOrUrl = abp.session.user.profilePictureId;
    }
